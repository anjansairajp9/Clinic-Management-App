...
# GET TREATMENT BY APPOINTMENT ID(PARTIAL), CREATE TREATMENT
...
# GET TREATMENT BY APPOINTMENT ID(Complete)
def get_treatment_by_appointment_id_full(db, clinic_id: int, appointment_id: int):
    with db.cursor() as cursor:
        cursor.execute(
            """SELECT
                    treatments.id AS id,
                ...
                WHERE treatments.clinic_id = %s
                AND treatments.appointment_id = %s
                AND treatments.is_active = TRUE
            """, (clinic_id, appointment_id))
