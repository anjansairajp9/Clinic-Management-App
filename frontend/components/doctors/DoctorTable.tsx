...
import type { Doctor, DoctorDetails } from "@/types/doctor";
...
export default function DoctorTable({ searchQuery, refreshKey }: Props) {
	...
	const [editingDoctor, setEditingDoctor] = useState<DoctorDetails | null>(null);
	const [isAppointmentModalOpen, setIsAppointmentModalOpen] = useState(false);
	const [selectedDoctorForAppointments, setSelectedDoctorForAppointments] = useState<DoctorDetails | null>(null);
	...
	useEffect(() => {
		const fetchDoctors = async () => {
			try {
				const response = await searchDoctors({ query: searchQuery, page, limit });
				setDoctors(response);
			} catch (error) {
				console.error(error);
			}
	...
	const handleView = async (doctorId: number) => {
		try {
	...
	const handleEdit = (doctor: DoctorDetails) => {
	...
	const handleEditSuccess = async () => {
		...
		try {
			const updatedDoctor = await getDoctorById(editingDoctor.id);
			...
			>
				...
				{isMobile ? (
					<div style={{ display: "flex", flexDirection: "column" }}>
						{doctors.map((doctor) => (
							<div key={doctor.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
								<DoctorRow doctor={doctor} onView={handleView} isMobile={isMobile} />
							</div>
						))}
					</div>
				) : (
					...
					>
						<thead>
							<tr>
								{["Doctor", "Phone", "Specialization", "Actions"].map((header) => (
									<th
										key={header}
										style={{
											textAlign: "left",
				...
				<DoctorAppointmentHistoryModal
					...
					doctorId={selectedDoctorForAppointments?.id ?? null}
